...
if (process.argv.length != 4) {
    ...
    console.log("USAGE: node blockchain.js -c <chaincode-dir-name>");
    console.log("ex: ");
    console.log("node blockchain.js -c chaincode");
    process.exit();
}
process.argv.forEach(function(val, index, array) {
    if (index == 2 && (!val || val != "-c")) {
        ...
        if (!val) {
            ...
            ccPath = process.env["GOPATH"]+"/bluemix/blockchaincode/"+val;
        }
    }
});
...
function copyCertificate() {
    ...
    fs.writeFileSync(__dirname+'/certificate.pem', fs.readFileSync(__dirname+'/certificate.pem'));
...
function enrollAndRegisterUsers() {
    ...
    chain.enroll(users[0].username, users[0].secret, function(err, admin) {
        ...
        var enrollName = "mdonthi"; //creating a new user
...
function deployChaincode(user) {
    ...
    deployRequest.chaincodePath = "chaincode";
...
function assetTransfer(user){
  var invokeRequest = {
      ...
      fcn: "set_user",
      // Parameters for the invoke function
      args: ["asset1", "maruthi donthi"]
  };

  var invokeSetUser = user.invoke(invokeRequest);

  //print the invoke results
  invokeSetUser.on('submitted', function(results){
    console.log(util.format("\n Successfully asset transfer invoke transaction submitted to chaincode: request=%j, response=%j", invokeRequest, results));
  });
  invokeSetUser.on('complete', function(results){
    console.log(util.format("\nSuccessfully completed chaincode invoke transaction: request=%j, response=%j", invokeRequest, results));
    queryAsset(user);
  });
  invokeTx.on('error', function(err) {
